...
const HandmadeCakeFinale = () => {
  ...
  return (
    <section style={{ padding: 'clamp(32px, 7vw, 56px) clamp(16px, 5vw, 36px)', borderTop: '2px dashed #C9A882', textAlign: 'center', position: 'relative' }}>
      {/* Paper Confetti Shower when candle is blown */}
      <AnimatePresence>
        {isCandleBlown && (
          <div style={{ position: 'absolute', inset: 0, overflow: 'hidden', pointerEvents: 'none', zIndex: 10 }}>
            {[...Array(36)].map((_, i) => {
              const colors = ['#E76F51', '#F4A261', '#E9C46A', '#2A9D8F', '#264653', '#EBD8BE', '#9B2226'];
      ...
      >
        {/* Hand-Drawn Birthday Cake with Matchstick Candle */}
        <div style={{ marginBottom: '32px', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
          ...
          <motion.div
            ...
            style={{
              ...
              height: '100px',
              borderRadius: '50%',
              backgroundColor: isCandleBlown ? 'var(--paper-cotton)' : '#FFFFFF',
          ...
          >
            {isCandleBlown 
              ? (language === 'hi' ? '🎉 मोमबत्ती बुझ गई! आपकी हर दुआ पूरी हो! 🎂' : '🎉 Candle Blown! May all your prayers be answered! 🎂') 
              : (language === 'hi' ? 'मोमबत्ती बुझाने और कागज़ की कन्फेटी उड़ाने के लिए टैप करें 🕯️' : 'Tap to blow out the candle & make a wish! 🕯️')}
          </span>
        </div>

        {/* Handwritten Blessing Note */}
        <div style={{
          maxWidth: '600px',
          ...
          boxShadow: '0 8px 24px rgba(43, 29, 14, 0.08)'
        }}>
          <p 
            className={language === 'hi' ? 'font-kalam' : 'font-caveat'}
            style={{
              fontSize: language === 'hi' ? '1.35rem' : '1.95rem',
              color: 'var(--ink-blue)',
              lineHeight: 1.6,
              fontWeight: 700,
              marginBottom: '12px'
        ...
        {/* Family Collage in Handmade Photo Frame */}
        <div style={{
          maxWidth: '440px',
        ...
        <div style={{ marginBottom: '36px' }}>
          ...
          <p 
            className={language === 'hi' ? 'font-kalam' : 'font-caveat'}
            style={{
              fontSize: language === 'hi' ? '1.3rem' : '2.2rem',
              color: 'var(--ink-blue)',
              fontWeight: 700
            }}
          >
            {language === 'hi' ? '— आपका प्यारा बच्चा ❤️' : '— With All My Love, Forever & Always ❤️'}
          </p>
        </div>

        {/* Handmade Stamp at bottom */}
        <div style={{
          ...
          <span 
            className={language === 'hi' ? 'font-kalam' : 'font-caveat'}
            style={{
              fontSize: language === 'hi' ? '1rem' : '1.45rem',
              color: 'var(--ink-crimson)',
              fontWeight: 700,
              letterSpacing: '0.04em'
            }}
          >
            ✂️ HANDMADE WITH LOVE FOR MAA · 100% HEARTFELT ✂️
          </span>
        </div>
      </motion.div>
    </section>
...
export default HandmadeCakeFinale;
